...
let client = new irc.Client(bot.network, bot.nick, {
    ...
    port: 'port' in bot ? bot.port : 6697,
    localAddress: 'bind' in bot ? bot.bind : null,
    debug: !!settings.debug,
    showErrors: 'showErrors' in bot ? !!bot.showErrors : true,
    autoRejoin: 'rejoin' in bot ? !!bot.rejoin : false,
    autoConnect: 'auto' in bot ? !!bot.auto : true,
    secure: 'secure' in bot ? !!bot.secure : true,
    selfSigned: 'signed' in bot ? !!bot.signed : false,
    certExpired: 'certExpired' in bot ? !!bot.certExpired : false,
    floodProtection: 'protect' in bot ? !!bot.protect : false,
    floodProtectionDelay: 'protectDelay' in bot ? bot.protectDelay : 1000,
    sasl: 'sasl' in bot ? !!bot.sasl : 0,
    retryCount: 'retry' in bot ? bot.retry : 0,
    retryDelay: 'delay' in bot ? bot.delay : 2000,
    stripColors: 'strip' in bot ? !!bot.strip : false,
    channelPrefixes: 'prefix' in bot ? bot.prefix : "&#",
    messageSplit: 'split' in bot ? bot.split : 1024,
    encoding: 'encoding' in bot ? bot.encoding : '',
